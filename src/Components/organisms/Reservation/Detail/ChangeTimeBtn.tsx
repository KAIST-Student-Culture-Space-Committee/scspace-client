...
export default function ChangeTimeBtn({
  rid,
  refetch,
  timeFrom,
  timeTo,
  ...
  const [dateFrom, setDateFrom] = useState<Date>(
    new Date(timeFromUnit.year, timeFromUnit.month, timeFromUnit.date),
  );

  const [dateTo, setDateTo] = useState<Date>(
    new Date(timeToUnit.year, timeToUnit.month, timeToUnit.date),
  );
          ...
          },
          {
            onSuccess: () => {
              toaster.success({
                ...
                description:
                  "The reservation time has been updated successfully.",
              });
              refetch();
            },
            onError: error => {
    ...
    >
      {!isCorrect && (
        <Text color="red.500" fontWeight="semibold">
          The selected time range is invalid. Please ensure that the start time

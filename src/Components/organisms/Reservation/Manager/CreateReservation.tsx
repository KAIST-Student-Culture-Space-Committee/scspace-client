...
export default function CreateReservation() {
  ...
  const { getTime, getDate, addDateDays } = dateUtils();
  ...
  const [submitLog, setSubmitLog] =
    useState<IReservationMultipleCreateResurt | null>(null);
  ...
  function submit() {
    ...
    resList.forEach(res => {
      ...
      _timeFrom = getDate(
        getTime(res.dateFrom) + getTime({ hour: res.hourFrom }),
      );
      _timeTo = getDate(getTime(res.dateTo) + getTime({ hour: res.hourTo }));

      for (i = 1; i < repeat; i++) {
        _timeFrom = addDateDays(_timeFrom, 7);
        _timeTo = addDateDays(_timeTo, 7);
        time.push({
  ...
  return (
    <Scroll>
      ...
      <Stack>
        ...
        <Grid templateColumns="repeat(6, 1fr)" gap={8} py={2}>
          <GridItem colSpan={{ base: 6, md: 3 }}>
            <SpaceForm setSpaceId={setSpaceId} setCheck={setCheck} />
          </GridItem>
          <GridItem colSpan={{ base: 6, md: 3 }}>
          ...
          <GridItem colSpan={6}>
            <RepeatForm count={repeat} setCount={setRepeat} />
          </GridItem>
          <GridItem colSpan={6}>
            <TitleForm title={title} setTitle={setTitle} />
          </GridItem>
          <GridItem colSpan={6}>
            <DescriptionForm description={dscrp} setDescription={setDscrp} />
          </GridItem>
          <GridItem colSpan={{ base: 6, md: 3 }}>
            <InnerPeopleForm count={inner} setCount={setInner} />
          </GridItem>
          <GridItem colSpan={{ base: 6, md: 3 }}>
            <OuterPeopleForm count={outer} setCount={setOuter} />
          </GridItem>
          <GridItem colSpan={6}>
          ...
          <GridItem colSpan={{ base: 6, md: 2 }}>
            <WorkerForm value={worker} setValue={setWorker} />
          </GridItem>
        </Grid>
        <Separator />
        {submitLog && (

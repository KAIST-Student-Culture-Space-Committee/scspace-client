...
export function DateSlot({
  ...
  const { getDateString, addDays } = dateUtils();
  const [isHovered, setIsHovered] = useState(false);
  const slotTime = addDays(startTime, date);
      ...
      >
        ...
        >
          <Text fontSize="sm" margin={0} padding={0}>
            {getDateString(slotTime)}
          </Text>
          <Separator borderColor={getBorderColor()} width={"100%"} />
          <Center maxW="100%" overflow="hidden">
            {drawnOrgName ? (
              <Text maxW="100%" truncate overflow="hidden" color={getColor()}>
                {drawnOrgName}
              </Text>
            ) : (
              <DataList.Root orientation={"horizontal"}>
                <DataListItem label="Priority 1">{orgCount[0]}</DataListItem>
                <DataListItem label="Priority 2">{orgCount[1]}</DataListItem>
                <DataListItem label="Priority 3">{orgCount[2]}</DataListItem>
              </DataList.Root>
            )}
          </Center>
        </Grid>

...
export default function PerformanceLotteryInfoModal({
  info,
  open,
  setOpen,
  refetch,
  ...
  const { createLotteryInfo, deleteLotteryInfo, updateLotteryInfo } =
    usePerformanceLotteryInfoAPI(info?.id || 0);
  ...
  return (
    <SimpleDialog open={open} setOpen={setOpen}>
      <Dialog.Header>
        <Stack>
          <Dialog.Title>공연집중기간 추첨 정보</Dialog.Title>
          {isError && (

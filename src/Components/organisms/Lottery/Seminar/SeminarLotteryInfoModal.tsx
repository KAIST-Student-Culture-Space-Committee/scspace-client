...
export default function SeminarLotteryInfoModal({
  info,
  open,
  setOpen,
  refetch,
  ...
  const { createLotteryInfo, deleteLotteryInfo, updateLotteryInfo } =
    useSeminarLotteryInfoAPI(info?.id || 0);

  const { getTime, getDateString, getDate, getNow } = dateUtils();
  ...
  const today = useMemo(() => getDate(getNow()), [getDate, getNow]);
  ...
  return (
    <SimpleDialog open={open} setOpen={setOpen}>
      <Dialog.Header>
        <Stack>
          <Dialog.Title>세미나실 정기 예약 추첨 정보</Dialog.Title>
          {isError && (
          ...
          <Wrap justify="center">
            <Card.Root>
              ...
              <Card.Body>
                <Stack align={"center"}>
                  <Card.Title>
                    {getDateString(getTime(dateLotteryStart))} ~{" "}
                    {getDateString(getTime(dateLotteryEnd))}
                  </Card.Title>
                  <Wrap justify="center">
                    <DatePicker
            ...
            <Card.Root>
              ...
              <Card.Body>
                <Stack align={"center"}>
                  <Card.Title>
                    {getDateString(getTime(dateStart))} ~{" "}
                    {getDateString(getTime(dateEnd))}
                  </Card.Title>
                  <Wrap justify="center">
                    <DatePicker
                      wrapperClassName="datepicker"
                  ...
                  {},
                  {
                    ...
                    onError: error => {
                      toaster.error({
                        title: "추첨 정보 삭제 실패",
                        description:
                          error.message || "추첨 정보 삭제에 실패했습니다.",
                      });
                    },
                  },
                );
                  ...
                  },
                  {
                    ...
                    onError: error => {
                      toaster.error({
                        title: "추첨 정보 업데이트 실패",
                        description:
                          error.message || "추첨 정보 업데이트에 실패했습니다.",
                      });
                    },
                  },
                );
                ...
                },
                {
                  ...
                  onError: error => {
                    toaster.error({
                      title: "추첨 정보 추가 실패",
                      description:
                        error.message || "추첨 정보 추가에 실패했습니다.",
                    });
                  },
                },
              );
